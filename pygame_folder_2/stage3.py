...
def stage_status(screen):
    ...
    # テキスト
    text_stage3=font_stage1.render("ステージ3", True, (255, 255, 255))
    text_stage3_english =font_impact3.render("Stage3", True, (255, 255, 255))
...
#当たり判定制御--------------------------------------------------------------------------------------------------------------
        ...
        new_enemy_bombs01=[]
        if enemy_arrive==True:
            for x,y in enemy_bombs01:
                y+= enemy_bombs_speed * math.sin(angle_rad) * -1
                x+= enemy_bombs_speed * math.cos(angle_rad) * -1
                new_enemy_bombs01.append([x,y])
                if x < 0 or width < x and y < 0 or height < y:
                    new_enemy_bombs01.remove([x,y])

            
            enemy_bombs01=new_enemy_bombs01#更新
    ...
    # 描画
        ...
        if first_switch==True:#最初だけ処理される　ステージ名表示
            for i in range(1, height,40):
                ...
                pygame.display.update()
            screen.blit(text_stage3,(total_width/6,height/2))
            screen.blit(text_stage3_english,(total_width/6,height/2 + 80))
            pygame.display.update()
            time.sleep(2.5)
