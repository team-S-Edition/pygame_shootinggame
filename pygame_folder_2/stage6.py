...
def stage_status(screen):
    ...
    enemy_bomb03_image=pygame.image.load("銃弾.png")
    angle = 180
    ...
    enemy_bomb03_image=pygame.transform.rotate(enemy_bomb03_image, angle)
    enemy_bomb03_image = pygame.transform.scale(enemy_bomb03_image, (25, 25))
    ...
    enemy03=pygame.image.load("中ボス.png")
    ...
    enemy03_x,enemy03_y= width // 2, 194
    ui_x,ui_y = width,0
    speed = 5
    low_speed = 2
    enemy_speed = 3
    ...
    # テキスト
    text_stage4=font_stage1.render("ステージ6", True, (255, 255, 255))
    text_stage4_english =font_impact3.render("Stage6", True, (255, 255, 255))
    ...
    setsumei_text3 = font_text.render("スペースでタイトル画面へ",True ,(255,255,255))
    ...
    victory_text = font_title.render("全ステージクリア", True, (255,255,255))
...
#---------------------------------------------------------------------------------------------------------------------
        ...
        hit_enemy03=pygame.Rect(enemy03_x,enemy03_y,64,64)
...
#当たり判定制御--------------------------------------------------------------------------------------------------------------
        for x,y in bombs:
            if hit_enemy01.colliderect(x,y,16,16):
                ...
                enemy2_arrive=False
                enemy3_arrive = False
            elif hit_enemy03.colliderect(x,y,16,16):
                gameclear_flag=True
                dokan.play()
                screen.blit(bomb_efect, (x - bomb_efect.get_width() // 2, y - bomb_efect.get_height() // 2))
                pygame.display.update()
        ...
        #敵３の弾
        for x,y in enemy_bombs03:
            if hit_myplane.colliderect(x,y,10,10):
                if now_time-start_time>2000 and first_switch==False:#バグ対策
                    gameover_flag=True
                    enemy_bombs03.remove([x,y])
                    myplane_arrive=False

        if now_time-last_bullet_time3>shot_delay_enemy03:
            last_bullet_time3=pygame.time.get_ticks()
            enemy_bombs03.append([enemy03_x,enemy03_y])
        #回転
        bullet_angle += 2
        angle_rad = math.radians(bullet_angle)

        new_enemy_bombs03=[]
        if enemy3_arrive==True:
            for x,y in enemy_bombs03:
                y+= enemy_bombs_speed * math.sin(angle_rad) * 8
                x+= enemy_bombs_speed * math.cos(angle_rad) * 4
                new_enemy_bombs03.append([x,y])
                if x < 0 or width < x and y < 0 or height < y:
                    new_enemy_bombs03.remove([x,y])

            
            enemy_bombs03=new_enemy_bombs03#更新
            
            for x, y in enemy_bombs03:
                screen.blit(enemy_bomb03_image, (x, y))
        ...
        if enemy03_movemode==0:
            enemy03_x-= enemy3_speed
        elif enemy03_movemode==1:
            enemy03_x+= enemy3_speed
    ...
    #*if myplane_x<0:
     ...
     #   myplane_x=width-myplane01.get_width()*\
        ...
        enemy03_x = max(0, min(enemy02_x, width - enemy02.get_width())) # 背景の端座標896
        enemy03_y = max(0, min(enemy02_y, height - enemy02.get_height()))
    ...
    # 描画
        ...
        if enemy3_arrive:
            screen.blit(enemy03,(enemy03_x,enemy03_y))
