...
def stage_status(screen):
    ...
    # テキスト
    text_stage2=font_stage1.render("ステージ2", True, (255, 255, 255))
    text_stage2_english =font_impact2.render("Stage2", True, (255, 255, 255))
    ...
    # 描画
        ...
        if first_switch==True:#最初だけ処理される　ステージ名表示
            for i in range(1, height,40):
                ...
                pygame.display.update()
            screen.blit(text_stage2,(total_width/6,height/2))
            screen.blit(text_stage2_english,(total_width/6,height/2 + 80))
            pygame.display.update()
            time.sleep(2.5)
